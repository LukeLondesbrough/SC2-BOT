...
class BtechHeroMarine(BotAI):
    async def on_step(self, iteration:int):
            ...
            if self.townhalls:
                  ...
                  if cc.is_idle and self.can_afford(UnitTypeId.SCV) and  cc.assigned_harvesters < 22:
                    cc.train(UnitTypeId.SCV)
                       ...
                       if self.can_afford(UnitTypeId.SUPPLYDEPOT):
                            target_depot = self.structures(UnitTypeId.SUPPLYDEPOT).closest_to(cc)
                            pos = target_depot.position.towards(self.enemy_start_locations[0], random.randrange(8, 15))
                            await self.build(UnitTypeId.SUPPLYDEPOT, near=pos)
                        
            if self.can_afford(UnitTypeId.BARRACKS) and self.structures(UnitTypeId.BARRACKS).amount < 2 and self.already_pending(UnitTypeId.SUPPLYDEPOT) < 2:
